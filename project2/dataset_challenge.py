...
class ImageStandardizer:
    ...
    def __init__(self) -> None:
        ...
        self.augmentation_transforms = transforms.Compose([
            ...
            transforms.RandomResizedCrop(size = 64, scale = (0.9, 1)),
    ...
    def augment(self, X: npt.NDArray) -> npt.NDArray:
        """Perform data augmentation."""
        augmented_images = []
        for img in X:
            pil_img = Image.fromarray(img)
            augmented_img = self.augmentation_transforms(pil_img)
...
def get_challenge(task: str, batch_size: int, **kwargs) -> tuple[DataLoader, str]:
    ...
    tr.X = standardizer.augment(tr.X)
